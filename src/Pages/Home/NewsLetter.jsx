...
const NewsLetter = () => {
  ...
  const handleSubmit = async (e) => {
        ...
        className="flex flex-col sm:flex-row gap-4 justify-center">
          <input
           ...
           placeholder={focused ? '' : 'Please enter your email to subscribe.. e.g-abc@gmail.com'}
            onFocus={handleFocus}
            className="w-full sm:w-[400px] px-4 py-3 text-base placeholder:text-base placeholder:text-gray-400 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#6366f1]"
             value={email}
    ...
    <span className="absolute inset-0 w-full h-full mt-1 ml-1 transition-all duration-300 ease-in-out bg-[#6366f1] rounded-md group-hover:mt-0 group-hover:ml-0"></span>
    <span className="absolute inset-0 w-full h-full bg-white rounded-md "></span>
    <span className="absolute inset-0 w-full h-full transition-all duration-200 ease-in-out delay-100 bg-[#6366f1] rounded-md opacity-0 group-hover:opacity-100 "></span>
    <span className="relative text-[#6366f1] transition-colors duration-200 ease-in-out delay-100 group-hover:text-white"> {loading ? 'Subscribing...' : submitted ? 'Subscribed' : 'Subscribe'}</span>
    </button>
        </form>
      </div>
    </section>
