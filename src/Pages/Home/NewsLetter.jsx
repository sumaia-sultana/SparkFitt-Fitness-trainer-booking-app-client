  ...
  const handleSubmit = async (e) => {
    ...
    return (
      ...
      <div className="max-w-2xl mx-auto text-center">
        <h2 className="text-2xl text-[#3624bf] font-bold mb-4">Spark Up Your Inbox — Subscribe & Stay with Us!</h2>
        ...
        className="flex flex-col sm:flex-row gap-4 justify-center">
          <input
           ...
           placeholder={focused ? '' : 'Please enter your email to subscribe.. e.g-abc@gmail.com'}
            onFocus={handleFocus}
            className="w-full sm:w-[400px] px-4 py-3 text-base placeholder:text-base placeholder:text-gray-400 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#3624bf]"
             value={email}
    ...
    <span className="absolute inset-0 w-full h-full mt-1 ml-1 transition-all duration-300 ease-in-out bg-[#3624bf] rounded-md group-hover:mt-0 group-hover:ml-0"></span>
    <span className="absolute inset-0 w-full h-full bg-white rounded-md "></span>
    <span className="absolute inset-0 w-full h-full transition-all duration-200 ease-in-out delay-100 bg-[#3624bf] rounded-md opacity-0 group-hover:opacity-100 "></span>
    <span className="relative text-[#3624bf] transition-colors duration-200 ease-in-out delay-100 group-hover:text-white"> {loading ? 'Subscribing...' : submitted ? 'Subscribed' : 'Subscribe'}</span>
    </button>
        </form>
      </div>
    </section>
